...
%      Version: $Id: gaussmix.m 2733 2013-02-25 21:29:19Z dmb $
%
...
if nargin<5 || ischar(v0)             % no initial values specified for m0, v0, w0
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~fulliv          % initializing with diagonal covariance
    ...
    for j=1:lp
        ...
        for il=2:nl                     % process the data points in chunks
            ...
            px=px./ps(wk,:);            % relative mixture probabilities for each data point (columns sum to 1)
            lpx(ii)=log(ps)+mx;
            pk=pk+px*wt;                % pk(k,1) effective number of data points for each mixture (could be zero due to underflow)
            sx=sx+px*xsw(ii,:);
            sx2=sx2+px*xs2(ii,:);
        end
...
if fv              % check if full covariance matrices were requested
    ...
    for j=1:lp
        ...
        px=px./ps(wk,:);                % relative mixture probabilities for each data point (columns sum to 1)
        lpx(ii)=log(ps)+mx;
        pk=px*wt;                       % effective number of data points for each mixture (could be zero due to underflow)
        sx=px*xsw(ii,:);
        ...
        for il=2:nl
            ...
            px=px./ps(wk,:);                % relative mixture probabilities for each data point (columns sum to 1)
            lpx(ii)=log(ps)+mx;
            pk=pk+px*wt;                    % effective number of data points for each mixture (could be zero due to underflow)
